...
if(process.argv[2]==='prepare') {
 ...
 const info=JSON.parse(await readFile(publicPath,'utf8'));let envelope;
 for(let n=0;n<90;n++) {
 ...
 try {
  await cp(path.resolve('dist/public-review'),path.join(target,'public'),{recursive:true});
  await rm(path.join(target,'public/netlify.toml'),{force:true});
  await cp(path.resolve('netlify'),path.join(target,'netlify'),{recursive:true});
  for(const filename of ['src/domain/channel-snapshot.ts','src/providers/youtube-batch.ts','src/lib/channels/registry.ts','src/lib/channels/sync-service.ts','src/lib/creator-data.ts']) {
   const destination=path.join(target,filename);await mkdir(path.dirname(destination),{recursive:true});await cp(path.resolve(filename),destination);
  }
  const rootPackage=JSON.parse(await readFile('package.json','utf8'));
  const dependencies=Object.fromEntries(['@netlify/blobs','@netlify/functions','zod'].map(name=>[name,rootPackage.dependencies[name]]));
  if(Object.values(dependencies).some(value=>!value))throw new Error('Server dependency lock missing');
  await writeFile(path.join(target,'package.json'),JSON.stringify({name:'tuberbot-channel-sync-runtime',private:true,type:'module',engines:{node:'24.x'},dependencies}));
  await writeFile(path.join(target,'netlify.toml'),'[build]\n  publish="public"\n  command=""\n[functions]\n  directory="netlify/functions"\n  node_bundler="esbuild"\n');
  const release={release:'youtube-sync-v1',revision:source,mode:'BUDGET_AND_INQUIRY',leadProvider:'NETLIFY_FORMS',livePayments:false,livePayouts:false,channelSync:{implemented:true,scheduleUtc:'10 18 * * *',statusEndpoint:'/api/channel-data',requiresServerApiKey:true}};
  await writeFile(path.join(target,'public/release.json'),JSON.stringify(release));
  ...
  console.log('Official Netlify deployment command completed. Verify actual deployed functions and schedule separately.');
 } catch(error) {
