...
async function respond(request: Request, fn: () => Promise<Response>): Promise<Response> {
  try { return await fn(); }
    ...
    if (error instanceof WorkspaceError) return noStoreJson({ error: { code: error.code, message: error.message } }, { status: 409 });
...
export function listProjects(request: Request): Promise<Response> {
  return respond(request, async () => {
    ...
    const rows = await sql<{ document: unknown; advertiser_id: string; creator_id: string }[]>`
      SELECT document, advertiser_id, creator_id FROM workspace_projects
      WHERE advertiser_id = ${auth.actor.userId} OR creator_id = ${auth.actor.userId}
      ORDER BY updated_at DESC, id LIMIT 31
    `;
...
export function createProject(request: Request): Promise<Response> {
  return respond(request, async () => {
    ...
    const document = await sql.begin(async (tx) => {
      // Lock per advertiser to serialize the project-count limit and idempotent creation.
      ...
      const creator = await tx<{ display_name: string }[]>`
        SELECT u.display_name FROM users u WHERE u.id = ${input.creatorUserId} AND u.status = 'ACTIVE' AND u.deleted_at IS NULL
        AND EXISTS(SELECT 1 FROM user_roles r WHERE r.user_id = u.id AND r.role = 'CREATOR') LIMIT 1
      `;
...
export function projectCommand(request: Request, id: string): Promise<Response> {
  return respond(request, async () => {
    ...
    const auth = await identity(request, true);
    const input = requestSchema.parse(await parseBoundedJson(request, 8192));
    const sql = getDatabase().queryClient;
    const result = await sql.begin(async (tx) => {
      const rows = await tx<{ document: unknown; advertiser_id: string; creator_id: string }[]>`
        SELECT document, advertiser_id, creator_id FROM workspace_projects WHERE id = ${id}
        AND (advertiser_id = ${auth.actor.userId} OR creator_id = ${auth.actor.userId}) FOR UPDATE
      `;
      ...
      const before = workspaceDocumentSchema.parse(row.document);
      const command = { ...input, actor, at: new Date().toISOString() };
      ...
      if (applied.state.contract && !(await deriveWorkspace(before)).contract) {
        await tx`INSERT INTO workspace_contract_records (project_id, version, snapshot, sha256) VALUES (${id}, ${applied.state.contract.version}, ${JSON.stringify(applied.state.contract)}::jsonb, ${applied.state.contract.sha256})`;
      }
      return applied;
    });
    return noStoreJson({ ...result, requestId: randomUUID() });
  });
}
