...
/** Shared, deterministic sandbox workflow. This module cannot charge or pay anybody. */
...
const money = z.string().regex(/^[1-9]\d{0,12}$/u).refine((v) => BigInt(v) <= 1_000_000_000_000n, "금액은 1원~1조 원입니다.");
export const termsSchema = z.strictObject({
  title: line(100), brand: line(80), category: line(40),
  adType: z.enum(["BRANDED", "PPL", "OFFLINE", "AFFILIATE"]), amountKrw: money,
  ...
  usage: z.enum(["CHANNEL_ONLY", "BRAND_ORGANIC", "PAID_MEDIA"]),
  usageDays: z.number().int().min(1).max(3650),
  taxBasis: z.enum(["INCLUDED", "EXCLUDED"]),
});
export type Terms = z.infer<typeof termsSchema>;
export const actionSchema = z.discriminatedUnion("type", [
  z.strictObject({ type: z.literal("PROPOSE"), terms: termsSchema }),
  z.strictObject({ type: z.literal("COUNTER"), terms: termsSchema }),
  ...
  z.strictObject({ type: z.literal("REVISE"), note }),
  z.strictObject({ type: z.literal("APPROVE"), disclosureChecked: z.literal(true) }),
  z.strictObject({ type: z.literal("PUBLISH"), url: z.string().max(2048) }),
  z.strictObject({ type: z.literal("CONFIRM") }),
  z.strictObject({ type: z.literal("DISPUTE"), note }),
  z.strictObject({ type: z.literal("MESSAGE"), note }),
  z.strictObject({ type: z.literal("CANCEL"), note }),
]);
export type WorkspaceAction = z.infer<typeof actionSchema>;
export const seedSchema = z.strictObject({
  id: z.string().uuid(), mode: z.enum(["LOCAL_REVIEW", "SERVER_SANDBOX"]),
  createdAt: iso, advertiserLabel: line(80), creatorLabel: line(80), feeBps: z.number().int().min(0).max(10000),
});
export const commandSchema = z.strictObject({
  key: z.string().uuid(), expectedRevision: z.number().int().min(0), at: iso,
  actor: z.enum(["ADVERTISER", "CREATOR"]), action: actionSchema,
});
...
export type WorkspaceState = {
  id: string; revision: number; phase: Phase; proposals: Proposal[]; contract: Contract | null;
  paymentMethod: string | null; revisionCount: number; hold: string | null;
  deliveries: { note: string; url: string; at: string }[]; publicationUrl: string | null;
  events: { sequence: number; actor: Party; type: WorkspaceAction["type"]; at: string; detail: string }[];
};
export class WorkspaceError extends Error {
  constructor(readonly code: string, message: string) { super(message); this.name = "WorkspaceError"; }
}
const reject = (message: string, code = "INVALID_TRANSITION"): never => { throw new WorkspaceError(code, message); };
export function publicHttpsUrl(input: string, youtubeOnly = false): string {
  ...
  if (youtubeOnly) {
    ...
    if (["youtube.com", "www.youtube.com", "m.youtube.com"].includes(url.hostname)) {
      id = url.pathname === "/watch" ? url.searchParams.get("v") : url.pathname.match(/^\/(?:shorts|live)\/([\w-]{11})\/?$/u)?.[1] ?? null;
...
export function newWorkspace(seed: z.input<typeof seedSchema>): WorkspaceDocument {
  return { schemaVersion: 1, seed: seedSchema.parse(seed), commands: [] };
}
function requirePhase(state: WorkspaceState, allowed: Phase[]): void {
  if (!allowed.includes(state.phase)) reject("현재 단계에서 실행할 수 없는 작업입니다.");
}
function requireActor(actual: Party, expected: Party): void {
  if (actual !== expected) reject(`${expected === "ADVERTISER" ? "광고주" : "크리에이터"} 역할의 작업입니다.`, "PARTY_REQUIRED");
}
async function reduce(state: WorkspaceState, command: WorkspaceCommand, seed: WorkspaceDocument["seed"]): Promise<WorkspaceState> {
  ...
  if (state.hold && !["MESSAGE"].includes(action.type)) reject("분쟁 보류 중입니다. 운영자 검토 전에는 진행하거나 지급할 수 없습니다.", "DISPUTE_HOLD");
  ...
  switch (action.type) {
    ...
    case "COUNTER": {
      if (action.type === "PROPOSE") { requirePhase(state, ["DRAFT"]); requireActor(actor, "ADVERTISER"); }
        ...
        if (!latest || latest.author === actor) reject("상대방의 제안에만 역제안할 수 있습니다.");
      }
      ...
      detail = `조건 v${next.proposals.length} · ${action.terms.amountKrw}원`;
      break;
    }
    case "ACCEPT": {
      requirePhase(state, ["NEGOTIATING"]);
      ...
      latest.accepted.push(actor);
      detail = `조건 v${latest.version} 개별 수락`;
      if (latest.accepted.length === 2) {
        const amount = BigInt(latest.terms.amountKrw);
        const fee = (amount * BigInt(seed.feeBps) + 5000n) / 10000n;
        ...
        next.contract = { version: latest.version, terms: structuredClone(latest.terms), canonical, sha256: await sha256(canonical), at: command.at, feeBps: seed.feeBps, feeKrw: fee.toString(), creatorKrw: (amount - fee).toString() };
        next.phase = "CONTRACTED";
      }
      break;
    }
    case "SANDBOX_PAY":
      requireActor(actor, "ADVERTISER"); requirePhase(state, ["CONTRACTED"]);
      if (!next.contract) reject("양측 합의된 계약이 없습니다.");
      next.paymentMethod = action.method; next.phase = "FUNDED";
      detail = `${action.method} 모의 확인 · 외부 PG 요청과 실제 청구 없음`;
      break;
    case "SUBMIT":
      requireActor(actor, "CREATOR"); requirePhase(state, ["FUNDED", "REVISION"]);
      next.deliveries.push({ note: action.note, url: action.url ? publicHttpsUrl(action.url) : "", at: command.at });
      next.phase = "REVIEW"; detail = action.note;
      break;
    case "REVISE":
      ...
      next.revisionCount += 1; next.phase = "REVISION"; detail = action.note;
      break;
    case "APPROVE":
      requireActor(actor, "ADVERTISER"); requirePhase(state, ["REVIEW"]);
      next.phase = "APPROVED"; detail = "콘텐츠 및 광고 표시 확인 후 승인";
      break;
    case "PUBLISH":
      requireActor(actor, "CREATOR"); requirePhase(state, ["APPROVED"]);
      next.publicationUrl = publicHttpsUrl(action.url, true); next.phase = "PUBLISHED";
      detail = next.publicationUrl;
      break;
    case "CONFIRM":
      requireActor(actor, "ADVERTISER"); requirePhase(state, ["PUBLISHED"]);
      next.phase = "SETTLEMENT_READY"; detail = "구매 확인 · 지급은 실행하지 않음";
      break;
    case "DISPUTE":
      requirePhase(state, ["CONTRACTED", "FUNDED", "REVIEW", "REVISION", "APPROVED", "PUBLISHED", "SETTLEMENT_READY"]);
      next.hold = action.note; detail = action.note;
      break;
    case "MESSAGE": detail = action.note; break;
    case "CANCEL":
      requireActor(actor, "ADVERTISER"); requirePhase(state, ["DRAFT", "NEGOTIATING"]);
      next.phase = "CANCELLED"; detail = action.note;
      break;
  }
  next.revision += 1;
  next.events.push({ sequence: next.revision, actor, type: action.type, at: command.at, detail });
  return next;
}
...
export async function appendWorkspaceCommand(input: unknown, rawCommand: unknown): Promise<{ document: WorkspaceDocument; state: WorkspaceState; replayed: boolean }> {
  ...
  const previous = document.commands.find((item) => item.key === command.key);
  const state = await deriveWorkspace(document);
  if (previous) {
...
export async function importWorkspaceFile(text: string): Promise<WorkspaceDocument[]> {
  ...
  for (const doc of value.workspaces) {
    if (doc.seed.mode !== "LOCAL_REVIEW") reject("서버 작업은 브라우저 파일로 가져올 수 없습니다.");
    await deriveWorkspace(doc);
  }
  return value.workspaces;
}
export function exportWorkspaceFile(workspaces: WorkspaceDocument[]): string {
  return JSON.stringify({ schemaVersion: 1, workspaces }, null, 2);
}
export function contractText(document: WorkspaceDocument, state: WorkspaceState): string {
  ...
  return ["튜버봇 광고 협업 계약 검토본", "모의 거래 기록이며 전자서명·실제 계약 체결 증명이 아닙니다.", "",
    `작업: ${document.seed.id}`, `광고주: ${document.seed.advertiserLabel}`, `크리에이터: ${document.seed.creatorLabel}`,
    `캠페인: ${terms.title}`, `브랜드: ${terms.brand}`, `광고 유형: ${AD_TYPES[terms.adType]}`, `제작물: ${terms.deliverable}`,
    `합의 금액: ${terms.amountKrw} KRW (부가세 ${terms.taxBasis === "INCLUDED" ? "포함" : "별도"})`, `납기: ${terms.deadline}`, `수정 한도: ${terms.revisionLimit}회`,
    `사용 범위: ${terms.usage}`, `사용 기간: ${terms.usageDays}일`, `수수료 가정: ${feeBps / 100}% / ${feeKrw} KRW`, `공제 후 참고 금액: ${creatorKrw} KRW (세무 조정 전)`,
    `합의 버전: ${state.contract.version}`, `내용 SHA-256: ${hash}`, "", "광고 표시, 지식재산권, 환불·분쟁·세무 조건과 계약 효력은 당사자와 운영자의 별도 검토 대상입니다.",
    "실제 PG 승인·출금·지급은 이 기록으로 실행되지 않습니다.",
  ].join("\n");
}
