...
export function CreatorTile({ creator }: { creator: LegacyCreator }) {
  return <article className={s.creatorCard}>
...
export function CustomerDirectory({ initialQuery="" }: { initialQuery?: string }) {
  ...
  const matches = useMemo(() => {
    const data = legacyCreators.filter((creator) => (category === "전체" || creator.categories.includes(category)) && [creator.name,creator.handle ?? "",creator.youtubeId,...creator.categories].some((value) => value.toLocaleLowerCase("ko-KR").includes(deferred)));
  ...
  return <div className={s.scope} data-testid="customer-directory"><div className={s.wrap}>
    ...
    <p className={s.note} role="status">{matches.length}개 채널 · 탐색 자료 확인 2026.08.02 · 실시간 통계 아님</p>
    <div className={s.section} style={{ paddingTop:20 }}>
      {matches.length ? <div className={s.creatorGrid}>{matches.map((creator) => <CreatorTile key={creator.legacyId} creator={creator} />)}</div> : <div className={s.empty}><h2>일치하는 채널이 없습니다.</h2><p>검색어를 줄이거나 다른 분야로 찾아보세요.</p><button className={s.secondary} type="button" onClick={() => { setQuery(""); setCategory("전체"); }}>검색 조건 초기화</button></div>}
      <p className={s.note}>이 목록은 채널 탐색 자료입니다. 제휴·입점·광고 수락을 의미하지 않습니다. 문의는 채널이 아닌 튜버봇 운영팀으로 접수됩니다.</p>
    </div>
  </div></div>;
}
