...
import { useChannelCatalog } from "@/components/channels/use-channel-catalog";
import { apiMetric, ChannelRecordLabel, ChannelUpdateNotice, usableRecord } from "@/components/channels/channel-data-ui";
...
export function CreatorTile({ creator }: { creator: LegacyCreator }) {
  ...
  return <article className={s.creatorCard}><button type="button" className={s.saveButton} aria-label={`${creator.name} 관심 채널 ${saved ? "저장 해제" : "저장"}`} aria-pressed={saved} onClick={() => shortlist.toggle(creator.legacyId)}><Bookmark /></button>
    <div className={s.creatorHead}><Avatar key={displayed.imageUrl} creator={displayed} /><div><h3><Link href={`/channel/${creator.legacyId}`}>{displayed.name}</Link></h3><p>{creator.categories.length ? creator.categories.join(" · ") : "분야 정보 미등록"}</p></div></div>
    <div className={s.creatorStats}><span>{record ? "YouTube 구독자" : "구독자 기록"}</span><strong style={{ overflowWrap:"anywhere",minWidth:0 }}>{record ? apiMetric(record,"subscriberCount",null) : `${subscriberLabel(creator.subscriberCount)}명`}</strong></div>
    <ChannelRecordLabel record={record} />
...
export function CustomerDirectory({ initialQuery="" }: { initialQuery?: string }) {
  ...
  const matches = useMemo(() => {
    ...
    if (sort === "subscribers") {
      const subscribers = (creator: LegacyCreator) => { const record=records.get(creator.youtubeId); return record ? record.data?.subscriberCount === null || !record.data ? -1n : BigInt(record.data.subscriberCount) : BigInt(creator.subscriberCount); };
  ...
  return <div className={s.scope} data-testid="customer-directory"><div className={s.wrap}>
    ...
    <div className={s.directorySummary}><strong role="status">{matches.length}개 채널{savedOnly ? " · 저장 목록" : ""}</strong><ChannelUpdateNotice /></div>
...
export function CustomerProfile({ creator }: { creator: LegacyCreator }) {
  ...
  return <div className={s.scope}><div className={s.wrap}>
    ...
    <header className={s.profileHead}><Avatar key={displayed.imageUrl} creator={displayed} /><div><h1>{displayed.name}</h1><p>{creator.youtubeId}</p><p>{creator.categories.join(" · ") || "분야 정보 미등록"}</p><ChannelRecordLabel record={record} /></div></header>
    <dl className={s.profileStats}>{([ ["구독자","subscriberCount",creator.subscriberCount],["공개 영상 수","videoCount",creator.videoCount],["누적 조회수","viewCount",creator.viewCount] ] as const).map(([label,field,archived]) => <div key={field}><dt>{label}{record ? " · YouTube" : " 기록"}</dt><dd>{apiMetric(record,field,archived)}</dd></div>)}</dl>
    <ChannelUpdateNotice />
    <div className={s.profileCopy}><h2>이 채널로 캠페인을 준비하고 있나요?</h2><p>관심 채널과 브랜드 목표를 함께 문의에 남겨 주세요. 콘텐츠 형식별 예산은 별도 계산 도구에서 기획 조건에 맞춰 확인할 수 있습니다.</p><div className={s.actions}><Link className={s.primary} href={`/inquiry?channel=${encodeURIComponent(creator.legacyId)}`}>이 채널을 담아 문의</Link><a className={s.secondary} href={`https://www.youtube.com/channel/${creator.youtubeId}`} rel="noopener noreferrer" target="_blank">YouTube에서 보기 ↗</a></div><p className={s.note}>YouTube 수치는 API 응답의 확인 시각을 따릅니다. API 미연결 시에는 2026.08.02 확인 보관 자료로 표시합니다. 분야 구분은 튜버봇의 탐색 분류이며 채널 제휴·섭외 가능 여부는 별도입니다. 채널에 문의가 자동 발송되지는 않습니다.</p><details className={s.formula}><summary>채널 자료 출처</summary><p><a className={s.textLink} href={creator.sourceListingUrl} rel="noopener noreferrer" target="_blank">기존 튜버봇의 공개 탐색 자료</a></p></details></div>
  </div></div>;
}
