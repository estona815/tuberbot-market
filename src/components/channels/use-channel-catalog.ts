...
type State = { catalog: ChannelCatalog | null; loading: boolean; failed: boolean };
const initial: State = { catalog: null, loading: false, failed: false };
let current = initial;
let checkedAt = 0;
...
/** Single in-memory cache for all cards; never write API metrics into localStorage or a bundle. */
async function ensureCatalog(id?: string, force = false): Promise<void> {
  if (pending) return pending;
  if (!force && Date.now() - checkedAt < 5 * 60_000) return;
  emit({ ...current, loading: true });
  pending = (async () => {
    try {
      ...
      emit({ catalog, loading: false, failed: catalog.status === "STORAGE_UNAVAILABLE" });
    } catch { emit({ ...current, loading: false, failed: true }); }
