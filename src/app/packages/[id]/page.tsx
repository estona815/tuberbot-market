...
export default async function PackageDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="package-detail page-shell">
      ...
      <aside className="package-detail__purchase" id="proposal"><span>예시 시작 가격</span><strong>{formatKrw(item.priceKrw)}</strong><p><CalendarIcon /> 샘플 제작 조건 · {item.leadTimeDays}일</p><button className="button button--full" disabled type="button">광고 제안 준비 중</button><button className="button button--quiet" disabled type="button">찜 기능 준비 중</button><small><ShieldIcon /> 샘플 데이터 기반 제품 프리뷰 · 실제 거래 및 결제 불가</small></aside>
